...
| Slice | Main deliverable and surfaces | Validation proof | Stop boundary |
|---|---|---|---|
| `W6.S2a` live delivery adapter contract + env config seam | `app/delivery/*`、`configs/delivery.yaml`、tests | route contract 显式区分 `local_durable` vs `env_gated_live`；targeted delivery tests pass | 不接真实 SaaS API |
| `W6.S2b` first vendor delivery smoke bridge | one vendor adapter smoke path + runtime/webhook glue + tests | env missing 时显式 rejected/deferred；env present 时最小 smoke path 有 direct proof | 不扩成 full rollout orchestration |
...
- `W6.S2a` 必须证明 delivery contract 显式区分 local durable route 与 live vendor seam
- `W6.S2b` 必须证明 env missing 时不会 silent success
