...
SOURCE_PLAN_PATH = PLAN_ROOT / "warning-agent-minimax-local-primary-real-adapter-2026-04-20_PLAN.md"
SOURCE_STATUS_PATH = PLAN_ROOT / "warning-agent-minimax-local-primary-real-adapter-2026-04-20_STATUS.md"
SOURCE_WORKSET_PATH = PLAN_ROOT / "warning-agent-minimax-local-primary-real-adapter-2026-04-20_WORKSET.md"
...
def test_machine_control_plane_readme_exposes_active_pack_and_local_mode_requirements() -> None:
    ...
    assert _bullet_values(_section(readme, "Current Active Slice")) == ["MM.RV1"]
...
def test_machine_pack_is_parser_compatible_and_stage_order_is_serial() -> None:
    ...
    assert stage_order[:3] == ["MM.S1a", "MM.S1b", "MM.S2a"]
    assert stage_order[-1] == "MM.RV1"
    assert stage_order.index("MM.S1a") < stage_order.index("MM.S1b") < stage_order.index("MM.S2a")

    for slice_id, owner in [
        ("MM.S1a", "execute-plan"),
        ("MM.RV1", "execution-reality-audit"),
    ]:
